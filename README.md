...
- **Python 3.9+** with pip (CI tests against 3.9 / 3.11 / 3.12; the GitHub Actions workflows pin 3.11)
...
|--------|---|---------|
| `SOURCE_TENANT_URL` | Yes | Source Dynatrace tenant URL |
| `SOURCE_TENANT_TOKEN` | Yes | Source tenant primary token — Platform Token (`dt0s16`) or classic API Token (`dt0c01`); header format auto-detected by prefix |
| `SOURCE_TENANT_API_TOKEN` | Optional | Source tenant classic API Token (`dt0c01`) — only when the primary is a Platform Token AND you manage v1.88.0-excluded resources (see [Getting API Tokens](#getting-api-tokens)) |
| `TARGET_TENANT_URL` | Yes | Target Dynatrace tenant URL |
| `TARGET_TENANT_TOKEN` | Yes | Target tenant primary token (same model as source) |
| `TARGET_TENANT_API_TOKEN` | Optional | Target tenant classic API Token (same role as source, target side) |
...
│   │   ├── dt_client.py            # Dynatrace API client with combined-auth routing
│   │   ├── config.py               # pipeline.yaml + .env loader
│   │   ├── logging_setup.py        # Logging
│   │   └── types.py                # TenantConfig, ExportResult, CONFIG_TYPE_API_MAP, etc.
│   ├── export_pipeline/            # Format generators, reconciliation, topology
│   └── deploy_pipeline/            # Format detector, deployers, analysis
│
├── tests/                          # pytest suite (Phase 03)
│   ├── conftest.py                 # Reusable tenant + client fixtures
│   ├── test_auth_header.py         # _header_for_token (Bearer / Api-Token)
│   ├── test_auth_routing.py        # v1.88.0 exclusion-list URL matching
│   ├── test_dt_client_auth.py      # _auth_header_for_url scenarios
│   ├── test_dt_client_http.py      # End-to-end HTTP (responses-mocked)
│   ├── test_tenant_config.py       # TenantConfig dataclass
│   └── test_config_loader.py       # PipelineConfig env + yaml loading
│
├── .github/workflows/              # GitHub Actions
│   ├── export.yml                  # Export workflow (workflow_dispatch)
│   ├── deploy.yml                  # Deploy workflow (workflow_dispatch)
│   └── test.yml                    # pytest matrix (Python 3.9 / 3.11 / 3.12)
│
├── scripts/                        # Legacy migration tools + utilities
│   ├── migrate.py                  # All-in-one Python migration (legacy)
│   ├── migrate.sh                  # All-in-one Shell migration (legacy)
...
│   └── citation-status.md          # URL liveness report (regenerated by validator)
│
├── .claude/                        # AI assistant instructions
│   ├── CLAUDE.md                   # Entry point
│   ├── DECISIONS.md                # Architectural decisions log
│   ├── architecture.md             # Project structure + data flow
│   ├── phases/                     # PHASE-NN-{active,done}.md
│   └── rules/                      # core, development, testing, deployment,
│                                   #   python, debugging, existing-code, reference-currency
│
├── pyproject.toml                  # pytest + coverage configuration
├── requirements.txt                # Runtime Python dependencies
├── requirements-dev.txt            # Development dependencies (pytest, responses, ...)
