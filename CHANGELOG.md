...
### Documentation
- **`docs/IAM_EXPORT_GUIDE.md`** — standalone manual procedure for bulk-exporting IAM as Terraform without using Claude or this repo's helper scripts. Walks a novice user through OAuth client creation, provider download via `terraform init`, identifying the binary path, env var setup, running the `-export` utility with the four IAM resource type names, reviewing output, and `terraform import`-ing into state. Includes a sample bash loop for batch imports, a troubleshooting table, and explicit caveats about point-in-time exports and bindings re-assignment behavior.
